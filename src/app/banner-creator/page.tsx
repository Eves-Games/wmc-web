...
const handleColourChange =
  (setFunc: Function, activeTab: string) =>
  (colour: string) =>
  (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    setFunc((prev: any) =>
      activeTab === "base" ? colour : { ...prev, colour },
    );
  };
...
const handlePatternChange =
  ...
  (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    setFunc((prev: any) =>
      !pattern ? null : { name: pattern, colour: prev?.colour || "black-dye" },
    );
  };

const handleTabChange =
  (setFunc: Function) => (tab: "base" | "primary" | "secondary") => {
    setFunc(tab);
  };
...
export default function Page() {
  ...
  const [secondary, setSecondary] = useState<ColouredPattern | null>(
    null,
  );
  const [activeTab, setActiveTab] = useState<"base" | "primary" | "secondary">(
    "base",
  );

  const handleBaseColourChange = useCallback(
    handleColourChange(setBaseColour, activeTab),
    [activeTab],
  );
  const handlePrimaryColourChange = useCallback(
    handleColourChange(setPrimary, activeTab),
    [activeTab, primary],
  );
  const handleSecondaryColourChange = useCallback(
    handleColourChange(setSecondary, activeTab),
    [activeTab, secondary],
  );
  const handlePrimaryPatternChange = useCallback(
    handlePatternChange(setPrimary),
    [],
  );
  const handleSecondaryPatternChange = useCallback(
    handlePatternChange(setSecondary),
    [],
  );
    ...
    <section className="space-y-4 text-center">
      ...
      <div className="flex flex-col items-center space-y-4">
        <div role="tablist" className="tabs tabs-lifted tabs-lg">
          <button
            role="tab"
            className={clsx("tab", activeTab === "base" && "tab-active")}
            onClick={() => handleActiveTabChange("base")}
          >
            Base
          </button>
          <button
        ...
        <PatternButtons
          ...
          handlePatternChange={
            activeTab === "primary"
              ? handlePrimaryPatternChange
              : handleSecondaryPatternChange
          }
        />
      </div>
    </section>
    ...
    <div className="grid w-fit grid-cols-8 gap-2">
      {colours.map((colour) => (
        <button
          ...
          onClick={handleColourChange(colour.name)}
        >
          <Image
            src={colour.imageSrc}
            width={50}
